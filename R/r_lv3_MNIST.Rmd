---
title: "人工智能知識及應用證書（兼讀制）: 模式識別"
output:
  html_notebook: default
--- 

Date : 01-Oct-2023 v2.1
...
### Prerequisite
* Students should complete 人工智能知識及應用證書（兼讀制）: 神經網路
...
```{r}
showImage(mnist_train, row_index=4)
```
...
leftSide=paste(colnames(inds), collapse = "+")
rightSide=paste(names(MNIST::mnist_train)[1:784], collapse = "+")
f <- as.formula(paste(leftSide, " ~ ", rightSide ))
f
```
Task 9: Preparation 
...
```{r}
showImage(mnist_test, row_index=4)
```

```{r}
preds <- neuralnet::compute(model, mnist_test[1, 1:784])
print(mnist_test[4, 'y'])
preds$net.result
